...
class StackOverflowExtractor:
    ...
    def fetch_search(self, type, number_of_answers, has_accepted_answer, nb_of_views):
        url = 'search/advanced'
        ...
        result = self.api.fetch(url, fromdate=from_date, todate=to_date, order=order, sort=sort, accepted=accepted, answers=answers, tagged=tagged, views=views, site='stackoverflow')
    ...
    def fetch_answers(self, question_ids, type):
        self.api.page_size = 100
        self.api.max_pages = 5
        sort = "activity"
        order = "desc"
        ...
        for i, sub_set in enumerate(question_ids):
            ...
            result = self.api.fetch(url, ids=ids, sort=sort, order=order, site='stackoverflow', filter='withbody')
